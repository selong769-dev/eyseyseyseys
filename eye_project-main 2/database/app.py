...
import requests
import qrcode
from flask import Flask, Response, request, jsonify, send_file, redirect, render_template
...
# 기존의 모든 app = Flask 코드를 지우고, 이 세팅 "딱 하나만" 남겨두세요!
app = Flask(
    __name__, 
    template_folder="/Users/s/Desktop/eye_project-main 2/web/templates", 
    static_folder="/Users/s/Desktop/eye_project-main 2/web/static"
)
...
def get_qr(session_id: int):
    ...
    # [맥북 가상 구동용] 대문 주소(/)로 들어오면 index.html 화면을 보게 만드는 코드((5/25))
# 기존 3줄짜리 index() 함수를 이 코드로 교체해 줍니다.
@app.route('/')
def index():
    import os
    # 파이썬이 지금 실제로 어떤 경로에서 index.html을 찾으려고 하는지 터미널에 찍어주는 디버깅용 코드
    print("\n" + "="*50)
    print("현재 Flask가 설정한 templates 폴더의 진짜 절대경로:")
    print(app.template_folder)
    print("그 폴더 안에 index.html 파일이 진짜 존재하나요?:", os.path.exists(os.path.join(app.template_folder, 'index.html')))
    print("="*50 + "\n")
